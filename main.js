...
// Add the history panes
let results = [];
let commands = [];
for (let i = 0; i < 5; i++) {
  let el = document.getElementById("r"+i);
  const o = setUpOutput(el, "", false, "3024-night");
  results.push(o);

  el = document.getElementById("c"+i);
  const p = setUpOutput(el, "", false, "yonce");
  commands.push(p);
}

// Set up the input and output panes.
const outputWrapper = setUpOutput(output, ps.pipeline.currentPipe().output(), false, "3024-night");
const inputWrapper = setUpOutput(input, await ps.pipeline.currentPipe().input(),
  ps.pipeline.currentPipeIndex() ? false : true, "3024-night");

// Set up the keyboard shortcuts and the editor panel options.
const keyMap = {
    "Ctrl-Enter": determineLanguageAndRun,
    "Alt-Up": ps.previousPipe,
    "Alt-ArrowUp": ps.previousPipe,
    "Alt-Down": ps.nextPipeOrInsertAfter,
    "Alt-ArrowDown": ps.nextPipeOrInsertAfter,
    ...
    "Alt-Right": ps.nextPipeline,
    "Alt-ArrowRight": ps.nextPipeline,
    "Alt-Left": ps.prevPipeline,
    "Alt-ArrowLeft": ps.prevPipeline,
...
ps.setPanes(editor, inputWrapper, outputWrapper, cmLangs, results, commands);
...
async function evaluatePython() {
  ...
  await import("./app/pyodide-py-worker.js").then(async (py) => {
    ...
    const ids = {id: runningPipe.id(), prevID: runningPipe.prevID()};

    const { results, error, output } = 
      await py.asyncRun(program, input, files.concat(["input.txt"]), ids);
    let stdout = '';
    if (error) {
      stdout += error;
    }
