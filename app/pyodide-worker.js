...
self.onmessage = async (event) => {
  ...
  const { id, python, stdin, files, ids } = event.data;
  stdout = '';
  ...
  if (ids.prevID in inputs) {
    pyodide.globals.set("io_input", inputs[ids.prevID]);
  }
  ...
  try {
    ...
    try {inputs[ids.id].destroy();} catch{}
    inputs[ids.id] = results;
